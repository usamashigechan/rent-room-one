...
def scrape_suumo_kanto_lines_stations():
    """
    SUUMOの関東全県の路線ページから県名、路線名（<a>タグ）、駅名（<span>タグ）、件数（span.searchitem-list-value、(X,XXX)形式を整数に変換）を取得
    """
...
if __name__ == "__main__":
    print("SUUMO関東全県・全路線・全駅データ抽出（県名、路線名：<a>タグ、駅名：<span>タグ、整数、(X,XXX)形式対応）")
    print("=" * 50)
    
    df = scrape_suumo_kanto_lines_stations()
    ...
    if df.empty:
        ...
        # CSVファイルとして保存
        df.to_csv('suumo_kanto_all_lines_stations.csv', index=False, encoding='utf-8-sig')
        print("CSVファイル 'suumo_kanto_all_lines_stations.csv' として保存しました。")